...
#     println(cut.cutfacets.ls_to_facet_to_inoutcut)
#   end
#   # @notimplementedif !isconsistent_bgcell_to_inoutcut(_cuts,partition(cell_gids))
...
function cut(cutter::Cutter,bgmodel::DistributedDiscreteModel{Dc},args...) where Dc
  gcids = get_face_gids(bgmodel,Dc)
  gfids = get_face_gids(bgmodel,Dc-1)
  ...
  @notimplementedif !isconsistent_bgcell_to_inoutcut(_cuts,partition(gcids))
  _cutfacets = map(local_views(cuts)) do cut
    cut.cutfacets 
  end
  @notimplementedif !isconsistent_bgcell_to_inoutcut(_cutfacets,partition(gfids)) 
  DistributedEmbeddedDiscretization(cuts,bgmodel)
end
